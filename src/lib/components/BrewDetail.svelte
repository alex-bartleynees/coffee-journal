<script lang="ts">
	...
	let { brew, bean, grinder, prevBrew, prevBean }: {
		brew: Brew;
		bean: Bean;
	...
	<div class="recipe-grid">
		...
		<div class="recipe-cell">
			...
			<div class="recipe-value">
				{brew.grindSetting}<span class="recipe-unit">{grinder?.name.split(' ')[0]}</span>
			</div>
		</div>
	</div>
	...
	{#if prevBrew && prevBean}
		<a class="compare-hint" href="/brew/{brew.id}/compare">
			<div>
				...
				<div class="compare-hint-value">{prevBean.name} · {prevBrew.rating}/10</div>
			</div>
